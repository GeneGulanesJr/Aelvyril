...
describe("shouldEnterSpecMode", () => {
  ...
  it("auto: long single sentence triggers", () => {
    const prompt =
      "build a complete authentication system with login, logout, password reset, email verification, OAuth integration, and admin role management that supports multiple tenants";
  ...
  it("GATEWAY_SPEC_HEURISTIC=off disables even in auto mode", () => {
    const prev = process.env.GATEWAY_SPEC_HEURISTIC;
    process.env.GATEWAY_SPEC_HEURISTIC = "off";
    try {
      expect(shouldEnterSpecMode("add a button and integrate the payment flow", "auto")).toBe(false);
    } finally {
      if (prev === undefined) delete process.env.GATEWAY_SPEC_HEURISTIC;
      else process.env.GATEWAY_SPEC_HEURISTIC = prev;
    }
  });
});