...
The Aelvyril gateway and the `lapis` container open the same `memory.db` file on a shared Docker volume. The Pi extension (gateway side) opens it directly via `better-sqlite3` (`LAPIS_HOME`-based) for the hot chat path; the `lapis` container runs the HTTP/MCP server over the same file for future memory-browser panels and external MCP clients.
...
- `journal_mode = WAL` set by the Pi extension at startup.
- `LAPIS_HOME=/data/lapis` set on every `pi --mode rpc` child (`extraEnv` in `Supervisor.prompt`); the path resolves inside both containers.
...
- The single-file model caps at v1 throughput. Multi-node scale-out would push us to a hosted Postgres with the same zod envelope shapes — code that consumes `EventEnvelope` doesn't change.
...
- The per-user namespace (`user:<clerkUserId>`) is a string in the LaPis `projects` table. Migrating to Postgres preserves the schema; the namespace lookup is indexable.

Status: accepted 2026-09-22 (spec: `docs/superpowers/specs/2026-09-22-aelvyril-agent-platform-design.md`, decision D8). The Phase 4 `infra/compose.yaml` wires both containers with the shared `lapis-data` volume.
