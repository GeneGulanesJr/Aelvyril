...
describe('CostTracker', () => {
  ...
  it('aggregates by agent type', () => {
    ...
    const report = tracker.getReport('ses_123');
    // supervisor: 1500 (no ticket) + 700 (#1) = 2200
  ...
  it('returns zero report for empty session', () => {
    const report = tracker.getReport('nonexistent');
    expect(report.total_tokens).toBe(0);
    expect(report.total_cost_usd).toBe(0);
    // All agent types present with zeros
    expect(Object.keys(report.by_agent)).toHaveLength(7);
  });
});
